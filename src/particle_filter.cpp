...
void ParticleFilter::updateWeights(double sensor_range, double std_landmark[],
  ...
  for (int i=0; i<num_particles; i++) {
    ...
    for(unsigned int j=0; j<transformed_obs.size(); j++) {
      double delta_x = transformed_obs[j].x - map_landmarks.landmark_list[transformed_obs[j].id-1].x_f;
      double delta_y = transformed_obs[j].y - map_landmarks.landmark_list[transformed_obs[j].id-1].y_f;

       particles[i].weight *= gaussian * exp(-(pow(delta_x, 2) / gauss_x + pow(delta_y, 2) / gauss_y));
    }
